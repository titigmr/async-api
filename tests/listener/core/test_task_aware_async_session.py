...
def test_session_proxy_call_to_inner_session():
    session_proxy = TaskAwareAsyncSession()
    # Random call to the subsequent object
    assert session_proxy.is_active == True

def test_create_session_outside_event_loop():
    session_proxy = TaskAwareAsyncSession()
...
@pytest.mark.asyncio
async def test_create_session_inside_event_loop():
...
@pytest.mark.asyncio
async def test_create_session_in_new_task():
...
@pytest.mark.asyncio
async def test_create_session_in_new_tasks():
    # One session per task
    session_proxy = TaskAwareAsyncSession()
