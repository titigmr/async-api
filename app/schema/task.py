...
class Callback(BaseModel):
    url: AmqpDsn | HttpUrl | None = Field(None, description="URL ou AMQP DSN à appeler en callback")
    type: Literal["http", "amqp"] | None = Field(None, description="Type de callback (http ou amqp)")
...
class TaskResponse(BaseModel):
    ...
    status: Literal["pending", "failure", "in_progress", "success"] = Field(
        default="success", description="Statut de la création de la tâche"
    )
