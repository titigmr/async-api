...
class TaskCountByStatusAndServiceView:
    def __init__(self, service: str, status: str, count: int):
...
class PendingAndRunningTaskView:
    def __init__(self, service: str, status: str, submition_date: str, start_date: str):
        self.service: str = service
        self.status: str = status
...
class MetricsTaskRepository:
    ...
    async def count_tasks_per_status_and_service(self):
        rows = await self.db.execute(select(Task.service,Task.status,func.count(Task.service)).group_by(Task.service,Task.status))
        res = list(map(lambda x: TaskCountByStatusAndServiceView(x[0],x[1],x[2]), rows.all()))
        return res
    
    async def running_and_pending_tasks(self):
        rows = await self.db.execute(
            select(Task.service,Task.status,Task.submition_date, Task.start_date)
                .where(or_(Task.status == TaskStatus.PENDING , Task.status == TaskStatus.RUNNING))   
        )
        res = list(map(lambda x: PendingAndRunningTaskView(x[0],x[1],x[2],x[3]), rows.all()))
        return res
