...
class ClientConfig:
    def __init__(
        ...
        self.authorizations: dict[str, ClientAuthorization] = authorizations or {}
...
class ClientConfigRepository:
    ...
    def load_clients_config(client_file: str):
        try:
            with open(file=client_file) as file:
                config = yaml.load(stream=file, Loader=yaml.SafeLoader)
                ClientConfigRepository.CLIENTS = (
                    ClientConfigRepository._parse_yaml_struct(config)
                )
    ...
    def _parse_yaml_struct(config) -> dict[str, ClientConfig]:
        ...
        for config_item in config:
            ...
            client_secret: str | None = ClientConfigRepository._resolve_secret(
            ...
            for auth in authorisations_part:
                ...
                authorisations[service_name] = ClientAuthorization(
                    service=service_name, quotas=quotas
                )
