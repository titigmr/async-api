...
class ServicesConfigRepository:
    ...
    def load_services_config(svc_file: str) -> None:
        try:
            ...
            raise ServicesConfigException(msg)
    ...
    def _parse_yaml_struct(config) -> dict[str, ServicesConfig]:
        ...
        if not isinstance(config, list):
            msg = "Invalid configuration format. Expected a list of services."
            raise ServicesConfigException(msg)
        ...
        for config_item in config:
            ...
            if "name" not in config_item:
                msg = "Each service configuration must contain a 'name' key."
                raise ServicesConfigException(msg)
    ...
    def _handle_json_schema(json_schema: str | None) -> dict[str, Any] | None:
        ...
        if json_schema is not None:
            try:
                ...
                msg = f"JSON schema file {json_schema} not found."
                raise ServicesConfigException(msg)
            except json.JSONDecodeError as e:
                msg = f"Error parsing JSON schema file {json_schema}: {e}"
                raise ServicesConfigException(msg)
        return None
    ...
    def all_services(self) -> dict[str, ServicesConfig]:
        ...
        if not ServicesConfigRepository.SERVICES:
            msg = "No services loaded. Please load the services configuration first."
            raise ServicesConfigException(msg)
