...
class AmqpNotifier(BaseNotifier):
    ...
    async def notify(self, callback: dict, message: dict):
        ...
        if amqp_callback is None:
            raise NotificationException(f"Amqp notifier cannot handle callback: {callback}")
        await self.notify_retry(amqp_callback, message, 0)

    async def notify_retry(self, amqp_callback: AmqpCallback, message: dict, retry: int):
        try:
