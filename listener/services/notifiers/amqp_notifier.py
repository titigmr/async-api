...
class AmqpNotifier(BaseNotifier):
    ...
    async def notify_retry(
        self, amqp_callback: AmqpCallback, message: dict, retry: int
    ):
        try:
            ...
            async with connection:
                ...
                await channel.default_exchange.publish(
                    pika_message, routing_key=amqp_callback.queue
                )
                logger.debug("Amqp notification send successfully.")

        except Exception as e:
            if retry < self.max_reties:
                wait_time: int = pow(retry + 1, 2)
                logger.debug(
                    f"Failure, about to retry n°{retry + 1}/{self.max_reties} in {wait_time}s..."
                )
            ...
            raise NotificationException(
                f"Error during the amqp call for callback: {amqp_callback}", e
            )
